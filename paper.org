...
:END:
#+title: Unpacking Complexity: Human Preference Guides Interpretable Combinatorial Solutions
...
#+LaTeX_HEADER:\shorttitle{Interpretable Combinatorial Solutions}
...
First, construct validity is a concern. We measured interpretability with a single question ("Which of the two solutions do you find easier to understand?"), and our conclusions depend on how participants understood this prompt and on the strategies they used to decide. Metrics like CC and VC index structural cues that we assume facilitate understanding. However, participants may have used these cues directly --- selecting solutions with neat order or exact fills because they were appealing --- without deeper comprehension [cite/p:@feldmanSimplicityPrinciplePerception2016; @vanderhelmSimplicityLikelihoodVisual2000]. Such cue-driven choices could conflate "easier" with "better," reducing construct specificity. Notably, the consistent influence of HC partly mitigates this concern, as favoring heuristic-aligned solutions implies some engagement with a solution beyond surface layout. A stronger test would complement preference with performance-based tasks that require using or completing partially completed solutions.

Second, ecological validity was constrained. Participants judged completed optimal solutions under unlimited time. Real deployments often involve partial solutions, constraints, risk, and deadlines; interpretability may then interact with accountability and trust [cite/p:@leeTrustAutomationDesigning2004; @dietvorstAlgorithmAversionPeople2015]. Third, we examined relatively small instances (4–6 bins, 7–9 items). Larger, more complex problems may amplify or shift which cues dominate interpretability. Fourth, our heuristic-related complexity (HC) metric instantiated a single greedy strategy (largest-bin, largest-item first); other human-plausible heuristics (e.g., best-fit decreasing; [cite//bare:@johnsonWorstcasePerformanceBounds1974;@coffmanApproximationAlgorithmsBin1997;@kellererKnapsackProblems2004]) may yield additional or alternative alignment signals. Finally, webcam-based gaze measurement limited spatial precision and precluded scanpath-level analyses.
