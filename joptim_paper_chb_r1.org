...
As shown in Figure ref:fig:complexity, we operationalize these properties with three solution-level metrics, introduced here at a high level and defined in detail in [[#methods][Methods]]. We selected these metrics because each maps onto a distinct, theory-grounded aspect of interpretability and each is directly actionable for optimization systems or interfaces. /Heuristic-related complexity/ (HC) captures heuristic alignment, /compositional complexity/ (CC) captures within-bin cognitive simplicity, and /visual-order complexity/ (VC) captures perceptual order in the display. More generic information-theoretic notions of complexity could also be considered, but they would be less directly tied to identifiable cognitive mechanisms and less immediately useful for system design. We do not claim that HC, CC, and VC exhaust all relevant determinants of interpretability; rather, they provide a theory-driven and operational starting set. As a visual-layout control, we include /diagonal dissimilarity/ (DD), a covariate that captures purely geometric similarity to an idealized diagonal-like assignment pattern.
...
While explainable planning offers methods for justifying and aligning plans with users' mental models, and work on predictive explanations has advanced substantially in XAI [cite//bare:@barredoarrietaExplainableArtificialIntelligence2020; @rudinStopExplainingBlack2019], empirical and feature-based accounts of interpretability for optimization solutions are still emerging [cite/p:@ibsHumanExplanationsExplainable2024]. Related work in other optimization domains likewise examines preferences between competing solutions without specifying solution-level structural metrics [cite/p:@kyritsisPerceivedOptimalityCompeting2022]. Our study addresses this gap within the packing paradigm by quantifying how three solution-level properties --- HC, CC, and VC --- predict human choices, response speed, and attention when comparing equally optimal solutions.
...
This metric assesses complexity based on the composition of items in each bin. Each bin is treated as the outcome of a generative model, and higher surprisal under that model is interpreted as greater complexity. A bin is characterized by three properties: (a) the number of items, $N$; (b) the vector of relative item sizes, $C$, which sums to one when the bin is nonempty; and (c) the unused capacity fraction, $E$. Assuming conditional independence, the joint density factorizes as
...
Bins with many items, intermediate fill levels, or less regular compositions therefore receive higher complexity scores than bins that are nearly empty, nearly full, or dominated by simpler combinations. For a single bin, the negative log-probability
...
\noindent quantifies bin complexity in nats. The resulting compositional complexity of a solution is the average surprise of its bins under this model. Full distributional details and calibration are reported in Appendix ref:sec:calibration. For the confirmatory study, calibration yielded a {{{s(desc_c,cc_params.empty_dist)}}} empty-space distribution with $\sigma=$ {{{f(desc_c,cc_params.empty_param,%.3f)}}}, $p=$ {{{f(desc_c,cc_params.geom_p,%.3f)}}}, and $\alpha=$ {{{f(desc_c,cc_params.comp_alpha,%.3f)}}}, using {{{s(desc_c,cc_params.dirichlet_corr)}}} Dirichlet correction. For statistical analyses, its signed right-left difference is denoted $\Delta\text{CC}$ and its absolute difference $|\Delta\text{CC}|$.

# TODO: instead: use a a figure to describe CC by showing the single distributions (optional: have 2 more panels that describe the other two metrics)

# #+begin_export latex
# \begin{figure}[htbp]
#   \centering
#   \includegraphics[width=0.25\textwidth]{~/Dropbox/data_export/joptim-analysis/pub/fig_cc_graphical_model.pdf}
#   \caption{\label{fig:cc-graphical-model}Graphical Model for Compositional Complexity}
#   \par\footnotesize\textit{Note}. Bayesian-network representation of the single-bin compositional complexity (CC) model. The number of items $N$ is drawn from a geometric prior. Conditional on $N$, the relative item sizes $C$ follow a symmetric Dirichlet distribution, and the unused capacity fraction $E$ follows a bimodal distribution over near-empty and near-full bins. The per-bin surprise $L$ in Equation~\ref{eqn-complexity-single-bin} is the negative log-probability of observing $(N, C, E)$.
# \end{figure}
# #+end_export
...
:CLASS: runin apa
:END:
This metric assesses participants' difficulty-weighted /problem-solving efficiency/ (PSE), combining solution optimality and reaction time across the seven problem-solving trials, with harder trials contributing more strongly to the final score. Full computational details are reported in Appendix ref:sec:pse-computation.
...
During the preparation of this work, the authors used various generative AI and language-processing tools (e.g., LLMs and translation software) solely to improve the language and clarity of the manuscript. Following the use of these tools, the authors reviewed and edited the text as needed and take full responsibility for the final content of this article.
...
*** Computation of Problem-Solving Efficiency (PSE)
:PROPERTIES:
:CUSTOM_ID: appendix-g-pse-computation
:END:
<<sec:pse-computation>>

Problem-solving efficiency (PSE) combines solution optimality and speed across the seven problem-solving trials, with harder trials contributing more strongly to the final score. For trial \(i\), unweighted efficiency was defined as
