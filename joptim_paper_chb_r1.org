...
#+LaTeX_HEADER:\authorsnames{}
#+LaTeX_HEADER:\authorsaffiliations{}
#+LaTeX_HEADER:\shorttitle{INTERPRETABILITY CRITERIA FOR COMBINATORIAL SOLUTIONS}
...
We conducted an exploratory study to refine metrics followed by a preregistered confirmatory study using the fixed metrics. We tested three preregistered hypotheses. H1: within a pair of equally optimal solutions, participants would prefer the option with lower HC, CC, and VC. H2: larger absolute between-solution differences in these metrics would be associated with faster decisions. H3: more complex solutions would attract relatively greater dwell time. DD served throughout as a visual-layout control. In the [[#results][/Results/]] section, we report findings from the confirmatory sample and compare them with those from the exploratory sample.
...
For the experiment, we randomly generated problem instances subject to several constraints. Instances contained 4--6 bins and 7--9 items, no item exceeded the largest bin, no bin was smaller than the smallest item, the ratio of total item size to total bin capacity lay between 0.8 and 1.0, and each instance admitted at least two distinct optimal solutions. These constraints were chosen to keep the task simple yet nontrivial and to reduce cases in which the optimal assignment would be nearly one-to-one and therefore trivial to evaluate. Optimal solutions were generated with the OR-Tools CP-SAT solver (~ortools~ 9.8.3296; [cite//bare:@cpsatlp]), using default solver settings, by iteratively excluding previously found optima until up to 100 distinct optimal solutions had been enumerated for an instance. A detailed description of stimulus generation is provided in Appendix ref:sec:stimulus-generation.
...
:CUSTOM_ID: measures
:END:
To make the data hierarchy explicit, we distinguish participant-level, problem-level, solution-level, and solution-pair-level variables. The primary predictors in the main analyses are solution-pair-level differences derived from single-solution complexity scores.
...
\noindent quantifies its surprise and therefore complexity --- in nats. With appropriate parameter settings, this formulation allows us to assign relatively low surprise to solutions consisting of few-item bins that are either nearly empty or nearly full and whose item sizes approximate symmetric compositions, while deviations from these simple patterns can be assigned higher surprise. The resulting compositional complexity of a solution is then defined as the average surprise of its bins under this model. Full distributional details and calibration are reported in Appendix ref:sec:calibration. For the confirmatory study, calibration yielded a {{{s(desc_c,cc_params.empty_dist)}}} empty-space distribution with $\sigma=$ {{{f(desc_c,cc_params.empty_param,%.3f)}}}, $p=$ {{{f(desc_c,cc_params.geom_p,%.3f)}}}, and $\alpha=$ {{{f(desc_c,cc_params.comp_alpha,%.3f)}}}, using {{{s(desc_c,cc_params.dirichlet_corr)}}} Dirichlet correction. For statistical analyses, its signed right-left difference is denoted $\Delta\text{CC}$ and its absolute difference $|\Delta\text{CC}|$.
...
:CLASS: runin apa
:END:
This problem-level metric was operationalized as the ratio of the sum of item sizes to the sum of all bin capacities. It ranged from 0.8 to 1.0 in the sampled problems, with higher values denoting tighter and therefore more difficult instances.
...
All three analyses used right–left differences in HC, CC, VC, and DD as focal predictors and drew on the same set of moderators or covariates: PD, PSI, PSE, HO, and MD. Across all models, MD was never entered together with VC because both are derived from the same underlying disorder scores. Choice models used signed standardized differences and allowed two-way interactions between each complexity predictor and each moderator; reaction-time models used absolute standardized differences without interactions; gaze models used signed standardized differences in binomial GLMMs on right-versus-left gaze counts.
...
# In case we need more detail about the model, we can use below:
# "Gaze was analyzed as a binomial response. Successes were defined as the number of gaze samples on the right solution ($R$), and the total number of gaze samples was the sum of samples on both the right and left solutions ($R+L$). This was modeled with a logit link, where $p=R/(R+L)$."
...
\par\footnotesize\textit{Note}. Panels correspond to the three complexity metrics (HC = heuristic-related complexity, CC = compositional complexity, VC = visual-order complexity) and the covariate diagonal dissimilarity (DD). Complexity differences are standardized (0 = no difference; 1 = one standard deviation). Colored lines give the model-predicted probability of the four behavioral responses (‘definitely left', ‘slightly left', ‘slightly right', ‘definitely right'); shaded ribbons denote 95\% confidence intervals.\end{figure}
#+end_export
...
Our results supported H1: all three complexity differences predicted choice, indicating reliable preference for simpler solutions. These convergent effects fit a simple cognitive account. Visual order helps the perceptual system produce short, rule-like descriptions (e.g., "largest first"), heuristic alignment enables immediate rationalization of how a solution was constructed, and compositional simplicity reduces encoding demands because near-empty, near-full, and low-item bins can be summarized quickly [cite/p:@feldmanSimplicityPrinciplePerception2016; @vanderhelmSimplicityLikelihoodVisual2000;@chaterReconcilingSimplicityLikelihood1996;@helmholtzTreatisePhysiologicalOptics1962; @gigerenzerHeuristicDecisionMaking2011; @swellerCognitiveLoadProblem1988]. Because HC, CC, and VC retained effects in the same model, the results also suggest that these metrics capture partially distinct aspects of what participants found interpretable rather than a single undifferentiated notion of simplicity.
...
Several alternative explanations merit consideration. Preferences may partly reflect visual fluency or aesthetic regularity rather than deeper structural understanding [cite/p:@feldmanSimplicityPrinciplePerception2016; @vanderhelmSimplicityLikelihoodVisual2000;@chaterReconcilingSimplicityLikelihood1996;@helmholtzTreatisePhysiologicalOptics1962]. Participants may also have favored solutions resembling their own solving strategies, and in this untimed comparative setting, "easier to understand" may partly have meant "easier to justify." However, the robust effect of heuristic alignment, which captures correspondence with a greedy construction rather than mere display geometry, argues against a purely superficial account. The DD control further helps separate diagonal visual form from heuristic-aligned structure.
...
Second, our measurements of interpretability and processing were themselves constrained. Participants could have interpreted the prompt ("Which of the two solutions do you find easier to understand?") differently, and some choices may have reflected visual appeal or agreement with personal strategies rather than a purer notion of understanding. Although the consistent influence of HC suggests engagement with solution structure beyond superficial visual cues, we cannot fully separate interpretability from liking or ease of justification in the present design. Our use of webcam-based eye tracking further introduced limited spatial precision, restricting fine-grained analyses such as scanpaths and likely reducing sensitivity to subtle attentional dynamics [cite/p:@papoutsakiWebgazerScalableWebcam2016].
...
Future work should prioritize stronger measurement and ecological validity. Beyond subjective preferences, performance-based paradigms in which participants complete partially finished solutions could yield task-based indices of usability such as accuracy and time. Richer subjective assessments could be obtained with dedicated questionnaires for perceived interpretability, cognitive load, and satisfaction [cite/p:@brooksSelfcompassionAmongstClients2012; @doshi-velezRigorousScienceInterpretable2017; @narayananHowHumansUnderstand2018;@afsarDesigningEmpiricalExperiments2023]. Integrating laboratory eye tracking and pupillometry would offer more sensitive process measures of attention and cognitive load related to HC, CC, and VC [cite/p:@ecksteinEyeGazeWhat2017; @gollanGazeQuantifyingConscious2025]. To better reflect real-world allocation and design tasks, future paradigms should also incorporate time pressure and dynamic constraints [cite/p:@leeTrustAutomationDesigning2004; @dietvorstAlgorithmAversionPeople2015].
...
The data, code, benchmark instances, and study materials that support the findings of this study will be made available in an online repository after peer review. Due to the double-anonymized review process, repository details are omitted at this stage.
...
\par\footnotesize\textit{Note}. Signed standardized differences used in the choice/gaze analyses ($\Delta$HC, $\Delta$CC, $\Delta$VC, $\Delta$DD) and Maximum Disorder (MD). CC$_{0}$ corresponds to the uncalibrated CC used in the exploratory study. Upper triangles show Pearson's $r$; diagonals show distributions (mean in red, standard deviations dashed).
\end{figure}
#+end_export
...
\par\footnotesize\textit{Note}. Absolute standardized differences used in the RT analysis ($|\Delta|$HC, $|\Delta|$CC, $|\Delta|$VC, $|\Delta|$DD) and Maximum Disorder (MD). CC$_{0}$ corresponds to the uncalibrated CC used in the exploratory study. Upper triangles show Pearson's $r$; diagonals show distributions (mean in red, standard deviations dashed).
\end{figure}
#+end_export
...
- [cite:@thompsonHumanBiasesLimit2021]
  - Investigates how human inductive biases shape cumulative cultural evolution in a continuous optimization task using transmission chains of Bayesian learners designing virtual arrowheads.
  - Shows experimentally that populations converge on systematically suboptimal solutions when the objective-function optimum is misaligned with participants' priors (inductive biases) over the design space.
  - Provides a formal model in which cultural transmission plus innovation corresponds to a stochastic gradient-descent-like process, where inductive bias acts as a regularizer pulling solutions toward intuitive regions.
  - Demonstrates that even when the underlying fitness landscape is simple and the global optimum is easy to find in principle, biased transmission limits cumulative optimization performance.
  - Supports the broader claim that human cognitive constraints and prior expectations can systematically steer which solutions are discovered, transmitted, and refined, even in ostensibly objective optimization settings.
