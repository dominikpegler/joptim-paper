...
** what are the weights in the efficiency equation and does RT unit influence the weights?
...
Participants were recruited via Prolific [cite/p:@palanProlificacASubjectPool2018]. Detailed information regarding the recruitment process and the participant sample can be found in the dedicated [[#participants][Participants]] section.
...
\noindent where \(S_{i,j}\) is the score obtained by participant \(j\), \(O_{i}\) is the optimal score for the problem instance in trial \(i\), and \(RT_{i,j}\) is the reaction time (RT) (ms). Trial-specific difficulty weights ($w_{i}$) were derived from group efficiency. Let $\bar{E}_{i}$ denote the mean $E$ across participants on trial $i$. We first normalized these trial-wise efficiencies within each trial file to obtain relative “easiness” weights that summed to one. We then converted these to “hardness” weights by inverting and renormalizing them so that harder trials (with lower mean efficiency) received proportionally larger weights. A participant's overall PSE ($\eta_{j}$) was the weighted sum
